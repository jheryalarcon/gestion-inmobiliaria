...
export default function CardPropiedad({ propiedad, onActualizarPropiedad }) {
    ...
    const handleReactivar = async () => {
        const confirmed = await new Promise((resolve) => {
            toast(
                '¿Reactivar esta propiedad y hacerla disponible nuevamente?',
                {
                    action: { label: 'Reactivar', onClick: () => resolve(true) },
                    cancel: { label: 'Cancelar', onClick: () => resolve(false) },
                    duration: 8000,
                    id: 'confirm-reactivar'
                }
            );
        });
        if (!confirmed) return;
        ...
        try {
            ...
            await import('axios').then(({ default: axios }) =>
                axios.patch(
                    `${import.meta.env.VITE_BACKEND_URL}/api/propiedades/${propiedad.id}/estado`,
                    { nuevoEstado: 'disponible' },
                    { headers: { Authorization: `Bearer ${token}` } }
                )
            );
            toast.success('Propiedad reactivada y disponible nuevamente.');
            if (onActualizarPropiedad) onActualizarPropiedad({ ...propiedad, estado_publicacion: 'disponible' });
        } catch (err) {
            toast.error(err?.response?.data?.mensaje || 'Error al reactivar la propiedad');
        } finally {
            ...
            <div className="p-5 flex flex-col flex-grow">
                ...
                <div className="flex justify-between items-center mt-auto">
                    ...
                    <div className="flex gap-0.5 sm:gap-1">
                        ...
                        {puedeEditar && (
                            propiedad.estado_publicacion === 'inactiva' ? (
                                <button
                                ...
                                >
                                    {/* Ícono: check con flecha circular (reactivar) */}
                                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                                    </svg>
                                </button>
